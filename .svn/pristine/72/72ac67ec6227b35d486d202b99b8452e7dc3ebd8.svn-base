<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SponsorAdsPreview.aspx.cs"
    MasterPageFile="~/AdminHome.master" Inherits="USPDHUB.Admin.SponsorAdsPreview" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="cphUser" runat="server">
    ...
    <script type="text/javascript">
        ...
        function IMageChanging(imgId) {

            var IsAdMob = $("#" + imgId).attr('value');
            if (IsAdMob == "False") {
                $("#" + imgId).attr('src', '../../images/Dashboard/On.png');
                $("#" + imgId).attr('value', 'True');
            }
            else {
                $("#" + imgId).attr('src', '../../images/Dashboard/Off.png');
                $("#" + imgId).attr('value', 'False');
            }
        }
        ...
        function UploadNewBannerAd(ordernumber, profileId, UserId, OrderNum) {
            document.getElementById('<%=hdnOrderNo.ClientID %>').value = ordernumber;
            location.href = "../../Admin/SponsorAdUpload.aspx?slot=" + ordernumber + "&profileId=" + profileId + "&UserId=" + UserId + "&OrderNum=" + OrderNum;
        }
        ...
        function EditBannerAd(bannerId, ordernumber, ProfileID) {
            location.href = "../../Admin/SponsorAdUpload.aspx?OrderNum=" + ordernumber + "&banid=" + bannerId + "&profileid=" + ProfileID;
        }
        ...
        function UpdateAppDisplay() {
            ...
            orderList = "";
            uncheckedList = "";
            //for geeting order numbers when no appdisplay button is present 
            $('input.appdisplaycheck:not([type=checkbox])').each(function () {
                uncheckedList = uncheckedList + "##" + this.id.replace("chk", "");
            });
            ...
            $('.imgtoggle').each(function () {
                ...
                //SubmitAppDisplay(this.id.replace("chk", ""), this.checked);
                toggleList = toggleList + "##" + this.id.replace("img", "") + "," + $(this).attr('value');
                orderList = orderList + "##" + this.id.replace("img", "");
            });
            document.getElementById("<%=hdnIsAdMob.ClientID %>").value = "";
            document.getElementById("<%=hdnIsAdMob.ClientID %>").value = toggleList;

            document.getElementById("<%=hdnorderList.ClientID %>").value = "";
            document.getElementById("<%=hdnorderList.ClientID %>").value = orderList;
                ...
                <tr>
                    <td valign="top">
                        <div style="color: #218C8D; font-size: 11px;">
                            ...
                            <asp:HiddenField ID="hdnProfileName" runat="server" />
                        </div>
                        <br />
                      <%--  User ID:
                        <%=UserID%>
                                    ...
                                    <table cellpadding="0" cellspacing="0">
                                        <tr>
                                            ...
                                            <td class="openrate" style="float: right; padding-right: 20px;">
                                                <a href="SponsorAdClickCountReport.aspx">Open Rate Report</a>
                                            </td>
                                        </tr>
                                        <tr>
                                        ...
                                        <tr>
                                            <td style="padding-right: 170px; padding-top: 25px;" align="center" class="style2">
                                                <divs>
                                             <asp:Button ID="btnDashoard" runat="server" Text="Cancel" Width="90px" Height="25px"
                                                    OnClick="btndashboard_OnClick" style="margin:10px;" />
                                                     <asp:Button ID="btnUpdate" runat="server" Text="Update" Width="70px" Height="25px"
                                                    OnClientClick="return UpdateAppDisplay()" OnClick="btnUpdate_Click" style="margin:10px;" /> 
                                                     <asp:Button ID="btnInsert" runat="server" Text="Update" Width="70px" Height="25px" 
                                                    OnClientClick="return UpdateAppDisplay()" OnClick="btnInsert_OnClick" style="margin:10px;"  />
                                            </div>
                                                ...
                                                <asp:HiddenField runat="server" ID="hdnIsAdMob" />
                                                <asp:HiddenField runat="server" ID="hdnorderList" />
                                            </td>
                                            <td>
                                            </td>
                                            <td colspan="2">
