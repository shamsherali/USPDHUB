...
{
    public partial class AgencyListingold : System.Web.UI.Page
    {
                        ...
                        if (captcha.CheckCaptcha(txtcaptcha.Text.Trim()))
                        {
                            ...
                            if (checkUser == 0)
                            {
                                if (checkAgency == 0)
                                {
                                    ...
                                    if (AffiliateInvID > 0)
                                    {
                                        //checkParentProfile = busobj.CheckParentProfileExists(txtParentName.Text.Trim());
                                        //if (checkParentProfile > 0)
                                    ...
                                    int inquiryID = objAgency.AddAgencyUser("",txtAgencyname.Text, txtEmail.Text.Trim(), txtFirstName.Text.Trim(), txtphonenumber.Text, date,
                                        txtRemarks.Text.Trim(), 0, txtTitle.Text.Trim(), ddlHow.SelectedValue, vertical, PromoCode, parentProfileID,
                                        AffiliateInvID == 0 ? (int?)null : AffiliateInvID, txtLastName.Text.Trim(), Country, "", "", "", "", txtWebSiteURL.Text.Trim());
                                    if (inquiryID > 0)
                                    {
                                        ...
                                        {
                                            InsertUserDetails(inquiryID, parentProfileID);
                                        }
                                    }
                                    else
        ...
        private void InsertUserDetails(int inquiryId, int? parentProfileID)
        {
            int profileID = 0;
            int userID = 0;
            string username = "";
