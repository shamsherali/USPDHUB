...
namespace USPDHUB
{
    public partial class ViewCalendar : System.Web.UI.Page
    {
        ...
        public int SchCalendarId = 0;
        public int UserID = 0;
        public int ProfileID = 0;
        public string CalendarPreview = string.Empty;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Request.QueryString["SchID"] != null)
                {
                    if (Request.QueryString["SchID"].ToString() != "")
                    {
                        ...
                        SchCalendarId = Convert.ToInt32(schID);
                    }
                }
            ...
            catch (Exception ex)
            {
                ...
                /*** Error Log ***/
                objInBuiltData.ErrorHandling("ERROR", "ViewCalendar.aspx.cs", "Page_Load", ex.Message, Convert.ToString(ex.StackTrace),
        ...
        public void GetEventDetails()
        {
            try
            {
                ...
                if (SchCalendarId != 0)
                {
                    dtSchEventDetails = objCalendarAddOn.GetCalendarDetailsBySchID(SchCalendarId);
                }
                else
                {
                    dtSchEventDetails = objCalendarAddOn.GetCalendarAddOnDetails(CalendarId);
                }
                if (dtSchEventDetails.Rows.Count > 0)
                {
                    ...
                    if (SchCalendarId != 0)
                    {
                        CalendarId = Convert.ToInt32(dtSchEventDetails.Rows[0]["CalendarId"].ToString());
                        ...
                        if (!string.IsNullOrEmpty(dtSchEventDetails.Rows[0]["ShareCalendar"].ToString()))
                        {
                            if (Request.QueryString["SI"] == null)
                            {
                                shareCalendar = dtSchEventDetails.Rows[0]["ShareCalendar"].ToString();
                            }
                        }
                    }
                    else
                ...
                CalendarPreview = "<html><head></head><body><table width='750px' border='0' cellspacing='0' cellpadding='0' align='center' style='border: solid 2px #F4EBEB;'><tr><td align='right' style='padding: 10px;' colspan='2'><a href='#' onclick='CloseWindow()'><img src='" + RootPath + "/images/Dashboard/btn-close.gif' border='0' /><a></td></tr><tr><td colspan='2' style='padding: 30px;'>" + eventBody + "</td></tr>";
                unsubscribeContent = UserProfileUnsubscribeLink(ProfileID, UserID);
                if (contactusFlag == true)
                {
                    string contactus = "<a href=\"" + RootPath + "/ContactUser.aspx?ID=" + EncryptDecrypt.DESEncrypt(UserID.ToString()) + "&SID=" + EncryptDecrypt.DESEncrypt(CalendarId.ToString()) + "&ET=CA\" target=\"_blank\"><img src='" + RootPath + "/images/Dashboard/contactus.gif' alt='Contact Us' border='0'/></a>";
                    CalendarPreview = CalendarPreview + @"<tr><td colspan='2' align='center' style='padding:10px;'>" + contactus + "</td></tr>";
                }
                if (Request.QueryString["SI"] != null)
                {
                ...
                if (shareCalendar != "")
                {
                    CalendarPreview = CalendarPreview + @"<tr><td colspan='2' align='center' style='padding-top: 5px;'><table border='0' cellpadding='0' cellspacing='0' width='100%'><tr><td align='center'>" + shareCalendar + "</td><td></td></tr></table></td></tr>";
                }
                CalendarPreview = CalendarPreview + @"<tr><td style='font-family: Arial, Helvetica, sans-serif; font-size: 11px; color: #666666; padding:10px; width:700px; border-top: solid 1px #F4EBEB;'>" + unsubscribeContent + "</td><td align='right' valign='top' style='font-family: Arial, Helvetica, sans-serif; font-size: 11px; color: #666666; border-top: solid 1px #F4EBEB;'><a href=\"" + RootPath + "\" target=\"_blank\"><img src='" + RootPath + "/images/VerticalLogos/" + DomainName + "emailby.gif' border='0' /></a></td></tr></table></body></html>";
                CalendarPreview = CalendarPreview.Replace("&#60;recipient's email address&#62;", EmailAddress);

                CalendarPreview = objCommon.ReplaceShortURltoHtmlString(CalendarPreview);
            }
            catch (Exception ex)
            {
                InBuiltDataBLL objInBuiltData = new InBuiltDataBLL();
                /*** Error Log ***/
                objInBuiltData.ErrorHandling("ERROR", "ViewCalendar.aspx.cs", "GetEventDetails", ex.Message, Convert.ToString(ex.StackTrace),
                    Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
            }
        }
        private void StoreLinks()
        {
            try
            {
                ...
                if (dtStoreLinks.Rows.Count > 0)
                {
                    ...
                    CalendarPreview = CalendarPreview + @"<tr><td style='padding:10px;' colspan='2' align='center'>" + Convert.ToString(dtStoreLinks.Rows[0]["StoreLinksTitle"]) + "</td></tr><tr><td style='padding:10px; padding:left: 50px;' colspan='2' align='center'>" + storelinks + "</td></tr>";
                }
            }
            catch (Exception ex)
            {
                InBuiltDataBLL objInBuiltData = new InBuiltDataBLL();
                /*** Error Log ***/
                objInBuiltData.ErrorHandling("ERROR", "ViewCalendar.aspx.cs", "StoreLinks", ex.Message, Convert.ToString(ex.StackTrace),
                    Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
            }
        }
        private string UserProfileUnsubscribeLink(int profileID, int userID)
        {
            ...
            try
            {
                ...
                unSubscribeLinkText = "This message was sent from " + profileName + " to &#60;recipient's email address&#62;. It was sent from: " + totalAddress + ". If you no longer wish to receive our updates, <a href=\"" + RootPath + "/UnsubscribeCalendar.aspx?ID=" + EncryptDecrypt.DESEncrypt(userID.ToString()) + "&SID=" + EncryptDecrypt.DESEncrypt(CalendarId.ToString()) + "\" target=\"_blank\">Click here</a> to unsubscribe.";
            }
            catch (Exception ex)
            {
                InBuiltDataBLL objInBuiltData = new InBuiltDataBLL();
                /*** Error Log ***/
                objInBuiltData.ErrorHandling("ERROR", "ViewCalendar.aspx.cs", "UserProfileUnsubscribeLink", ex.Message, Convert.ToString(ex.StackTrace),
                    Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
            }
