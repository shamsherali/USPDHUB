...
namespace USPDHUB
{
    public partial class OnlineBulletin : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    if (Request.QueryString["TID"] != null && Request.QueryString["TID"] != "")
                        BulletinID = Convert.ToInt32(EncryptDecrypt.DESDecrypt(Request.QueryString["TID"].ToString()));
                }
                ...
                if (!IsPostBack)
                {
                    if (BulletinID > 0)
                    {
                        ...
                        if (dtBulletin.Rows.Count > 0)
                        {
                            ...
                            //bulletinHeader = GetHeaderForOnlineBulletins(UserID, ProfileID);
                            string templateTitle = "";
                            templateTitle = Convert.ToString(dtBulletin.Rows[0]["Template_Name"]);
            ...
            catch (Exception ex)
            {
                ...
                /*** Error Log ***/
                objInBuiltData.ErrorHandling("ERROR", "OnlineBulletin.aspx.cs", "Page_Load", ex.Message, Convert.ToString(ex.StackTrace),
        ...
        public string GetConfigSettings(string pProfileID, string pType, string pConfigName)
        {
            ...
            catch (Exception ex)
            {
                InBuiltDataBLL objInBuiltData = new InBuiltDataBLL();
                /*** Error Log ***/
                objInBuiltData.ErrorHandling("ERROR", "OnlineBulletin.aspx.cs", "GetConfigSettings", ex.Message, Convert.ToString(ex.StackTrace),
                    Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
            }
            return returnValue;
        }
