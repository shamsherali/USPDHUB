...
public partial class Business_MyAccount_SendEvent : BaseWeb
{
    ...
    BusinessUpdatesBLL objBusUpdateBll = new BusinessUpdatesBLL();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            BusinessEventUsageCount = Convert.ToInt32(BusinessEventUsageCountstr.Replace(",", ""));
            ...
            if (!IsPostBack)
            {
                ...
                if (String.IsNullOrEmpty(Session["BusinessEventID"].ToString()))
                    Session["BusinessEventID"] = "0";

                string flyerPath = HttpContext.Current.Server.MapPath("~") + "\\Upload\\Events\\" + ProfileID.ToString();
                flyerPath = flyerPath + "\\" + Session["BusinessEventID"].ToString() + ".jpg";
                ...
                else
                {
                    ...
                    lblUpdatethumb.Text = "<img src='" + RootPath + "/Upload/Events/" + ProfileID.ToString() + "/" + Session["BusinessEventID"].ToString() + ".jpg?Guid=" + imageDisID + "' border='0' width='200' height='160'/>";
                }
        ...
        {
            //Error 
            objInBuiltData.ErrorHandling("ERROR", "SendEvent.aspx.cs", "Page_Load", ex.Message,
    ...
    protected void lnkimportcontacts_Click(object sender, EventArgs e)
    {
        ...
        {
            //Error 
            objInBuiltData.ErrorHandling("ERROR", "SendEvent.aspx.cs", "lnkimportcontacts_Click", ex.Message,
            Convert.ToString(ex.StackTrace), Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
        }
    }
    ...
    private int CheckAvailableTimeAndEmailCount(DateTime dateSent, DateTime dateNow)
    {
                ...
                if (txtemailsperday.Text != "" && ddltime.SelectedIndex > 0)
                {
                    ...
                    if (checkBusinessEventCount == 0)
                    {
                        ...
                        if (availableEmailCount >= 0)
                        {
                            ...
                            if (sendingdate == "3AM")
                            {
                                currentDateTime = businessEventSendingDate.AddHours(3);
                            }
                            else if (sendingdate == "9AM")
                            {
                                currentDateTime = businessEventSendingDate.AddHours(9);
                            }
                            else if (sendingdate == "3PM")
                            {
                                currentDateTime = businessEventSendingDate.AddHours(15);
                            }
                            else if (sendingdate == "9PM")
                            {
                                currentDateTime = businessEventSendingDate.AddHours(21);
                            }
#endif
                            if (dateSent <= dateNow)
                            {
                    ...
                    {
                        lblEmailsch.Text = "The schedule of another business event is clashing with the dates of this business event. Please either choose a different date or click the \"Business Event History\" link above to view the details of all the scheduled business event.";
                        showFlag = 0;
                    }
                }
                else
        ...
        {
            //Error 
            objInBuiltData.ErrorHandling("ERROR", "SendEvent.aspx.cs", "CheckAvailableTimeAndEmailCount", ex.Message,
            Convert.ToString(ex.StackTrace), Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
        }
        return showFlag;
    }
    ...
    protected void lnkSchEmail_Click(object sender, EventArgs e)
    {
        ...
        {
            //Error 
            objInBuiltData.ErrorHandling("ERROR", "SendEvent.aspx.cs", "lnkSchEmail_Click", ex.Message,
            Convert.ToString(ex.StackTrace), Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
        }
        //}
        //else
    ...
    {
        string url = Page.ResolveClientUrl("~/Business/MyAccount/ManageEventsCalendar.aspx");
        if (AppStatus == 1)
            url = Page.ResolveClientUrl("~/Business/MyAccount/ManageEventsCalendar.aspx?App=1");
        return url;
    }
    protected void lnkSendmail_Click(object sender, EventArgs e)
    {
        ...
        {
            //Error 
            objInBuiltData.ErrorHandling("ERROR", "SendEvent.aspx.cs", "lnkSendmail_Click", ex.Message,
            Convert.ToString(ex.StackTrace), Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
        }
    }
    ...
    private void SendEventSchedule(DateTime dateSent, DateTime dateNow)
    {
        try
        {
            ...
            if (totalAddress.Length > 0)
            {
                ...
                SchHisID = Convert.ToInt32(Session["BusinessEventID"].ToString());
                                        ...
                                        if (toEmailAdd != "")
                                        {
                                            ...
                                            if (optFlag == 0)
                                            {
                                                ...
                                                if (dupflag == 1)
                                                {
                                                    ...
                                                    objEventCalendarBll.InsertBusinessEventScheduleDetails(SchHisID, ProfileID, UserID, subject, toEmailAdd, schDate, schDate.Date, 0, false, groupID, contactusChecked, shareEvent, CUserID, domainID, chkStoreLinks.Checked);
                                                    countSchEmails++;
                                                    CountTotalEmails++;
                                                }
                                            }
                                        ...
                                        if (re.IsMatch(emailsent))
                                        {
                                            ...
                                            if (optFlag == 0)
                                            {
                                                ...
                                                if (dupflag == 1)
                                                {
                                                    ...
                                                    dtDupContacts.Rows.Add(dr);
                                                    objEventCalendarBll.InsertBusinessEventScheduleDetails(SchHisID, ProfileID, UserID, subject, emailsent, schDate, schDate.Date, 0, false, groupID1, contactusChecked, shareEvent, CUserID, domainID, chkStoreLinks.Checked);
                                                    countSchEmails++;
                                                    CountTotalEmails++;
                                                }
                                            }
        ...
        {
            //Error 
            objInBuiltData.ErrorHandling("ERROR", "SendEvent.aspx.cs", "SendEventSchedule", ex.Message,
            Convert.ToString(ex.StackTrace), Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
        }
    }
    ...
    private void SendMailtoSelectedContacts(DateTime dateSent)
    {
                                            ...
                                            if (BusinessEventUsage < BusinessEventUsageCount)
                                            {
                                                ...
                                                if (optFlag == 0)
                                                {
                                                    ...
                                                    if (dupflag == 1)
                                                    {
                                                        ...
                                                        dr["Receiver_Email"] = toEmailAdd;
                                                        dtDupContacts.Rows.Add(dr);
                                                        objEventCalendarBll.InsertBusinessEventScheduleDetails(SchHisID, ProfileID, UserID, subject, toEmailAdd, schCurrent, DateTime.Now.Date, 0, true, groupID, contactusChecked, "", CUserID, domainID, chkStoreLinks.Checked);
                                                        BusinessEventUsage++;
                                                        sendCount++;
                                                    }
                                                    // *** End Issue 986 *** //
                                            ...
                                            if (BusinessEventUsage < BusinessEventUsageCount)
                                            {
                                                ...
                                                if (optFlag == 0)
                                                {
                                                    ...
                                                    if (dupflag == 1)
                                                    {
                                                        ...
                                                        dr["Receiver_Email"] = emailsent;
                                                        dtDupContacts.Rows.Add(dr);
                                                        objEventCalendarBll.InsertBusinessEventScheduleDetails(SchHisID, ProfileID, UserID, subject, emailsent, schCurrent, DateTime.Now.Date, 0, true, groupID, contactusChecked, "", CUserID, domainID, chkStoreLinks.Checked);
                                                        BusinessEventUsage++;
                                                        sendCount++;
                                                    }
        ...
        {
            //Error 
            objInBuiltData.ErrorHandling("ERROR", "SendEvent.aspx.cs", "SendMailtoSelectedContacts", ex.Message,
            Convert.ToString(ex.StackTrace), Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
        }
    }
    ...
    {
        string urlinfo = Page.ResolveClientUrl("~/Business/MyAccount/EventsCalendar.aspx?EventId=" + Session["BusinessEventID"].ToString());
        Response.Redirect(urlinfo);
    }
    ...
    protected void btnpreview_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string shareEvent = string.Empty;
            DataTable dtCalendarEventDetails = objEventCalendarBll.GetCalendarEventDetails(Convert.ToInt32(Session["BusinessEventID"].ToString()));
            if (dtCalendarEventDetails.Rows.Count > 0)
            {
                ...
                eventTitle = dtCalendarEventDetails.Rows[0]["EventTitle"].ToString();
                ...
                contactus = "<a href=\"" + RootPath + "/ContactUser.aspx?ID=" + EncryptDecrypt.DESEncrypt(UserID.ToString()) + "&SID=" + EncryptDecrypt.DESEncrypt(Session["BusinessEventID"].ToString()) + "&ET=EC\"><img src='" + RootPath + "/images/Dashboard/contactus.gif' alt='Contact Us' border='0'/></a>";

                previewHtml = "<html><head></head><body><table width='100%' border='0' cellspacing='0' cellpadding='0' style='border: solid 2px #F4EBEB;'<tr><td colspan='2' style='padding:10px;'>" + eventTitle + "</td></tr> <tr><td colspan='2' style='padding:10px;'>" + dtEventStartDate + " to " + dtEventEndDate + "</td></tr><tr><td colspan='2' style='padding:20px;'>" + previewHtml + "</td></tr>";
                previewHtml = previewHtml + @"<tr><td colspan='2' align='right' style='padding: 5px;'><a style='color: #FBA862; font-weight: bold;' href='" + RootPath + "/Profiles/default.aspx?PID=" + ProfileID.ToString() + "&EC=yes\" target=\"_blank\"><img src='" + RootPath + "/images/viewcalendar.png' alt='View Calendar' border='0'/></a></td></tr>";
                ...
                if (shareEvent != "")
                {
                    previewHtml = previewHtml + @"<tr><td colspan='2' align='center' style='padding-top: 5px;'><table border='0' cellpadding='0' cellspacing='0' width='100%'><tr><td align='center'>" + shareEvent + "</td><td></td></tr></table></td></tr>";
                }
        ...
        {
            //Error 
            objInBuiltData.ErrorHandling("ERROR", "SendEvent.aspx.cs", "btnpreview_Click", ex.Message,
            Convert.ToString(ex.StackTrace), Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
        }
    }
    ...
    protected void lnktestbusinessUpdate_Click(object sender, EventArgs e)
    {
        ...
        {
            //Error 
            objInBuiltData.ErrorHandling("ERROR", "SendEvent.aspx.cs", "lnktestbusinessUpdate_Click", ex.Message,
            Convert.ToString(ex.StackTrace), Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
        }
    }
    ...
    protected void btntestsend_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string dtEventStartDate = string.Empty;
            string dtEventEndDate = string.Empty;
            dtEventDetails = objEventCalendarBll.GetCalendarEventDetails(Convert.ToInt32(Session["BusinessEventID"].ToString()));
            if (dtEventDetails.Rows.Count > 0)
            {
                ...
                unUsbscribeLink = UserProfileUnsubscribeLink();
                contactus = "<a href=\"" + RootPath + "/ContactUser.aspx?ID=" + EncryptDecrypt.DESEncrypt(UserID.ToString()) + "&SID=" + EncryptDecrypt.DESEncrypt(Session["BusinessEventID"].ToString()) + "&ET=EC\" target=\"_blank\"><img src='" + RootPath + "/images/Dashboard/contactus.gif' alt='Contact Us' border='0'/></a>";
                previewHtml = "<html><head></head><body><table width='100%' border='0' cellspacing='0' cellpadding='0' style='border: solid 2px #F4EBEB;'><tr><td colspan='2' align='center' style='height: 30px; padding: 7px;'>Problems viewing? Click <a href=\"" + RootPath + "/ViewEventCalendar.aspx?SID=" + EncryptDecrypt.DESEncrypt(Session["BusinessEventID"].ToString()) + "&REA=" + EncryptDecrypt.DESEncrypt(txttestemail.Text.Trim()) + "&SI=#ShowSocialIcons#\"><span style='color: Green;'>here</span></a> to view it online.</td></tr><tr><td colspan='2' style='padding:10px;'>" + eventTitle + "</td></tr> <tr><td colspan='2' style='padding:10px;'>" + dtEventStartDate + " to " + dtEventEndDate + "</td></tr><tr><td colspan='2' style='padding:20px;'>" + previewHtml + "</td></tr>";
                if (chkContactus.Checked == true)
                {
                ...
                if (result)
                {
                    ...
                    lblmess.Text = "Your test event has been sent successfully.";
                }
            }
        }
        catch (Exception ex)
        {
            //Error 
            objInBuiltData.ErrorHandling("ERROR", "SendEvent.aspx.cs", "btntestsend_Click", ex.Message,
            Convert.ToString(ex.StackTrace), Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
        }
    }
    ...
    private int CheckEmailsCountForEvent(DateTime dateSent)
    {
        ...
        try
        {
            ...
            checkSchEventEmailCount = objEventCalendarBll.CheckEventSendingCountforSendingDate(UserID, sendingDate);
        ...
        {
            //Error 
            objInBuiltData.ErrorHandling("ERROR", "SendEvent.aspx.cs", "CheckEmailsCountForEvent", ex.Message,
            Convert.ToString(ex.StackTrace), Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
        }
        return retCountNews;
    }
    ...
    private string UserProfileUnsubscribeLink()
    {
        ...
        try
        {
            ...
            unSubscribeLinkText = "This message was sent from " + profileName + " to &#60;recipient's email address&#62;. It was sent from: " + totalAddress + ". If you no longer wish to receive our events, <a href='" + RootPath + "/UnsubscribeEvent.aspx?ID=" + EncryptDecrypt.DESEncrypt(UserID.ToString()) + "&SID=" + EncryptDecrypt.DESEncrypt(Session["BusinessEventID"].ToString()) + "' target='_blank'>Click here</a> to unsubscribe.";
        }
        catch (Exception ex)
        {
            //Error 
            objInBuiltData.ErrorHandling("ERROR", "SendEvent.aspx.cs", "UserProfileUnsubscribeLink", ex.Message,
            Convert.ToString(ex.StackTrace), Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
        }
    ...
    protected void btnclick_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            int checkSch = 0;
            dtToday = objCommon.ConvertToUserTimeZone(ProfileID);
            checkForSelectedDateCount = objEventCalendarBll.CheckEventSendingCountforSendingDate(UserID, dtToday.Date);
            checkSch = objEventCalendarBll.CheckforBusinessEventSchedule(UserID);
            int checkCam = 0;
        ...
        {
            //Error 
            objInBuiltData.ErrorHandling("ERROR", "SendEvent.aspx.cs", "btnclick_Click", ex.Message,
            Convert.ToString(ex.StackTrace), Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
        }
    }
    ...
    protected int ValidateemailIDs(string address)
    {
        ...
        {
            //Error 
            objInBuiltData.ErrorHandling("ERROR", "SendEvent.aspx.cs", "ValidateemailIDs", ex.Message,
            Convert.ToString(ex.StackTrace), Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
        }
    ...
    protected void Validateradiobutton()
    {
        ...
        {
            //Error 
            objInBuiltData.ErrorHandling("ERROR", "SendEvent.aspx.cs", "Validateradiobutton", ex.Message,
            Convert.ToString(ex.StackTrace), Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
        }
    }
    ...
    protected void ValidateCampaign(DateTime dateSent, DateTime dateNow)
    {
        try
        {
            ...
            if (dateSent.Date >= dateNow.Date)
            {
                int checkForSelectedDateCount = 0;
                int checkSch = 0;
                checkForSelectedDateCount = objEventCalendarBll.CheckEventSendingCountforSendingDate(UserID, dateSent);
                checkSch = objEventCalendarBll.CheckforBusinessEventSchedule(UserID);
                int checkCam = 0;
                if (checkSch > MaxBusEventsCampaigns)
                {
        ...
        {
            //Error 
            objInBuiltData.ErrorHandling("ERROR", "SendEvent.aspx.cs", "ValidateCampaign", ex.Message,
            Convert.ToString(ex.StackTrace), Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
        }
    }
    ...
    {
        Business_MyAccount_SendEvent bms = new Business_MyAccount_SendEvent();
        int userID1 = Convert.ToInt32(userid);
        int checkForSelectedDateCount = 0;
        EventCalendarBLL cal = new EventCalendarBLL();
        CommonBLL objCommon = new CommonBLL();
        DateTime dtToday = objCommon.ConvertToUserTimeZone(Convert.ToInt32(HttpContext.Current.Session["ProfileID"].ToString()));

        checkForSelectedDateCount = cal.CheckEventSendingCountforSendingDate(userID1, dtToday.Date);
    ...
    {
        if (this.objUtilitiesBll == null)
        {
            this.objUtilitiesBll = new UtilitiesBLL();
        }
        if (this.objEventCalendarBll == null)
        {
            this.objEventCalendarBll = new EventCalendarBLL();
        }
        if (this.objBusUpdateBll == null)
        {
            this.objBusUpdateBll = new BusinessUpdatesBLL();
        }
        if (this.objBusinessBll == null)
        {
            this.objBusinessBll = new BusinessBLL();
        }


    }
    // *** Issue 1167 *** //
    protected void lnkGeneralDerails_Click(object sender, EventArgs e)
    {
        Response.Redirect(Page.ResolveClientUrl("~/Business/MyAccount/ModifyProfileDetails.aspx"));
    }
}
