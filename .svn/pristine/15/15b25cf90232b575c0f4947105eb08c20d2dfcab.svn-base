...
namespace USPDHUB.Business.MyAccount
{
    public partial class Bulletin_Imagegallery : BaseWeb
    {
        ...
        protected void btnSystemImages_Click(object sender, EventArgs e)
        {
            try
            {
                Response.Redirect(Page.ResolveClientUrl("~/Business/MyAccount/SystemImageGallery.aspx?imgSrc=" + Session["imgSrc"].ToString()));
            }
            catch (TimeoutException ex)
            {
                // Timeout doesn't get caught. Must be a different type of timeout.
                // So far, timeout is only caught by HttpException.
                objInBuiltData.ErrorHandling("ERROR", "Bulletin_Imagegallery.aspx.cs", "btnSystemImages_Click - timeout is only caught by HttpException.", ex.Message,
                Convert.ToString(ex.StackTrace), Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
            }
            catch (HttpException ex)
            {
                ...
                // }
                //Error 
                objInBuiltData.ErrorHandling("ERROR", "Bulletin_Imagegallery.aspx.cs", "btnSystemImages_Click - Took too long. Please upload a smaller image.", ex.Message,
                Convert.ToString(ex.StackTrace), Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
            }
            catch (Exception ex)
            {
                //Error 
                objInBuiltData.ErrorHandling("ERROR", "Bulletin_Imagegallery.aspx.cs", "btnSystemImages_Click", ex.Message,
                Convert.ToString(ex.StackTrace), Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
            }
        }
    }
