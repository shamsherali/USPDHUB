...
namespace USPDHUB.OP.inschoolhubcom
{
    public partial class AgencyListing : System.Web.UI.Page
    {
                    ...
                    if (dtAffiliate.Rows.Count > 0)
                    {
                        ...
                        else
                        {
                            ...
                            if (Convert.ToBoolean(dtAffiliate.Rows[0]["IsActivated"].ToString()))
                                status = "2"; // *** 2 means affiliate invitation has been activated *** //
                ...
                if (IsValid)
                {
                    if (txtEmail.Text.Contains(".lv") == false)
                    {
                        if (captcha.CheckCaptcha(txtcaptcha.Text.Trim()))
                        {
                            ...
                            int checkUser = busobj.CheckUserNameandPasswordForVaildUser(txtEmail.Text.Trim(), vertical, Country);
                                ...
                                if (checkAgency == 0)
                                {
                                    ...
                                    if (isPromo)
                                    {
                                        ...
                                        if (inquiryId > 0)
                                        {
                                            ...
                                            //string urlRedirect = secureRootPath + "/OP/" + Session["VerticalDomain"] + "/Enhance.aspx?InqID=" + EncryptDecrypt.DESEncrypt(inquiryId.ToString());
                                            // 3 means requested for branded app & subcription
                                            ...
                                            //if (PromoCode != string.Empty)
                                            //    urlRedirect = urlRedirect + "&PC=" + PromoCode;
                                            //if ((promoPercent == 100 && promoDuration > 0) || (hdnIsPaid.Value != "" && Convert.ToBoolean(hdnIsPaid.Value)))
        ...
        {
            Response.Redirect(Session["RootPath"] + "/OP/" + Session["VerticalDomain"] + "/Default.aspx");
        }
        ...
        private void InsertTransctionDetails(int InquiryId, int? parentProfileID)
        {
            try
            {
                ...
                if (dtInquiryDetails.Rows.Count > 0)
                {
                    ...
                    DataTable dtuserdetails = busobj.GetuserdetailsByProfileID(profileId);
                    ...
                    SendActivationEmail(Convert.ToString(Session["EmailID"]), password, "", Convert.ToString(Session["VerticalDomain"]));
                    objCommon.GenerateInvoice(profileId, "inschoolhub", DomainName);
        ...
        private int InsertAgencyDetails(int? parentProfileID)
        {
            ...
            int InquiryID = objAgency.AddAgencyUser(salesCode, txtAgencyname.Text, txtEmail.Text.Trim(), txtFirstName.Text.Trim(), txtphonenumber.Text, date,
                remarks, 0, title, ddlHow.SelectedValue, vertical, PromoCode, parentProfileID, AffiliateInvID == 0 ? (int?)null : AffiliateInvID, txtLastName.Text.Trim(), Country, txtAddress.Text.Trim(),
                                        txtCity.Text.Trim(), txtState.Text.Trim(), txtzipcode.Text.Trim(), "", Convert.ToBoolean(hdnIsLite.Value));
