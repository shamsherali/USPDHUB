...
namespace USPDHubClientAlerts
{
    public partial class Login : Form
    {
        ...
        public string countryVerticalName = Convert.ToString(ConfigurationManager.AppSettings.Get("CountryVerticalName"));
        ...
        public Login()
        {
            ...
            if (Properties.Settings.Default.IsLoginTrayIcon == true)
            {
                Properties.Settings.Default.IsLoginTrayIcon = false;
        ...
        void Form1_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                this.Text = DomainName + " Message Center - [Login]";

                /*
                string IconPath = @"Icons/" + ConfigurationManager.AppSettings.Get("FolderName") + "/Tips.ico";
                IconPath = Path.GetFullPath(IconPath);
                if (IconPath.ToLower().Contains("\\bin\\release"))
                {
                    IconPath = IconPath.Replace("\\bin\\Release", ""); 
                }
        ...
        private void LoginChecking(string username, string passcode)
        {
            ...
            else
            {
                ...
                dtobj = objService.GetUserDetails(username, countryVerticalName);
                if (dtobj != null)
                {
                    ...
                    {
                        dtAssociate = objService.GetAssociateUserDetails(username, countryVerticalName);
                    ...
                    if (dtobj.Rows.Count == 1)
                    {
                        if (Convert.ToBoolean(dtobj.Rows[0]["Active_flag"]) == true)
                        {
                            ...
                            if (bustabobj.Rows.Count == 1)
                            {
                                ...
                                if (countryVerticalName == domainVertical)
                                {
                                    int code = 1;
                                    string passcod = string.Empty;
                                    if (C_USER_ID != string.Empty) //added by venkat
                                    {
                                    ...
                                    if (code == 0)
                                    {
                                        ...
                                        {
                                            Utilities.AgencyName = Convert.ToString(bustabobj.Rows[0]["Profile_name"]);
                                            ...
                                            else if (bustabobj.Rows.Count == 1)
                                            {
                                                ...
                                                if (DtSubscriptionDetails.Rows.Count > 0)
                                                {
                                                    ...
                                                    try
                                                    {
                                                        if (Convert.ToString(DtSubscriptionDetails.Rows[0]["subscription_renewal_date"]).Trim() != string.Empty)
                                                            RenewalDate = Convert.ToDateTime(DtSubscriptionDetails.Rows[0]["subscription_renewal_date"]);
                                                    }
                                                    catch (Exception ex)
                                                    {
                                                        objService.ErrorHandling("ERROR", "Login.cs", "LoginChecking(Line: 266 )", Convert.ToString(ex.Message),
                                                            Convert.ToString(ex.StackTrace), Convert.ToString(ex.InnerException), Convert.ToString(ex.Data), "Tips Manager");

                                                        urlinfo = "";
                                                        OpenWebsiteBrowser(urlinfo);
                                                    }
                                                        ...
                                                        if (C_USER_ID == null || C_USER_ID.ToString() == "")
                                                        {
                                                            ...
                                                            if (dtobjforpasswprd.Rows.Count > 0)
                                                            {
                                                                if (dtobjforpasswprd.Rows[0]["Password_Changed"].ToString().Length > 0)
                                                                {
                                                                    ...
                                                                    {
                                                                        urlinfo = ConfigurationManager.AppSettings.Get("RootPath") + "/Business/MyAccount/Changepassword.aspx?CPD=ActivateUser";
                                        ...
                                        if (CheckRenewalValue == 0 && flag == 0)
                                        {
                                            ...
                                            {
                                                string urlinforforroot = "";//ConfigurationManager.AppSettings.Get("RootPath") + "/Business/MyAccount/Default.aspx";
                                                //Response.Redirect(urlinforforroot);
                                                OpenWebsiteBrowser(urlinforforroot);
                                            }
                                        }
                                    }
                                    else
        ...
        private void OpenWebsiteBrowser(string urlinfo)
        {
            #region remember me
            try
            {
            ...
            Notification alertsWindow = new Notification(UserID, ProfileID, UserName, Password, C_USER_ID);
            notifyIcon1.Visible = false;
            this.Close();
            alertsWindow.loginWindow = this;
            //alertsWindow.Show();


            UserDetails objUserDetails = new UserDetails();
            objUserDetails.ProfileID = ProfileID;
            objUserDetails.UserID = UserID;
            objUserDetails.Username = UserName;
            objUserDetails.Password = Password;
            objUserDetails.C_USER_ID = C_USER_ID;

            //MDIParentPage parent = new MDIParentPage(objUserDetails);
            //parent.loginWindow = this;
            //parent.Show();
            //parent.BringToFront();

            alertsWindow.objParentMDI = new MDIParentPage(objUserDetails);
            alertsWindow.objParentMDI.loginWindow = this;
            alertsWindow.objParentMDI.Show();
            alertsWindow.objParentMDI.BringToFront();
        }
        ...
        private void GetFreeUserDetails()
        {
            ...
            if (dtobj.Rows.Count > 0)
            {
                ...
                {
                    urlinfo = ConfigurationManager.AppSettings.Get("RootPath") + "/Business/MyAccount/Default.aspx";
                    //Response.Redirect(urlinfo);
                    OpenWebsiteBrowser(urlinfo);
                }
                else
        ...
        {
            USPDHubClientAlerts.Properties.Settings.Default.Reset();
        }
