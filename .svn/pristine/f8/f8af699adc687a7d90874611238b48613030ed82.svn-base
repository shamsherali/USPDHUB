    ...
    public partial class ViewItem : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Request.QueryString["SchID"] != null)
                {
                    if (Request.QueryString["SchID"].ToString() != "")
                    {
                        ...
                        SchCustomID = Convert.ToInt32(schID);
                    }
                }
            ...
            catch (Exception ex)
            {
                ...
                /*** Error Log ***/
                objInBuiltData.ErrorHandling("ERROR", "ViewItem.aspx.cs", "Page_Load", ex.Message, Convert.ToString(ex.StackTrace),
        ...
        public void GetBulletinDetails()
        {
            try
            {
                ...
                if (SchCustomID != 0)
                {
                    dtSchBulletinDetails = objAddOn.GetItemDetailsBySchID(SchCustomID);
                }
                else
                {
                    dtSchBulletinDetails = objAddOn.GetCustomModuleByID(CustomID);
                }
                if (dtSchBulletinDetails.Rows.Count > 0)
                {
                    ...
                    if (SchCustomID != 0)
                    {
                        ...
                        CustomID = Convert.ToInt32(dtSchBulletinDetails.Rows[0]["Custom_ID"].ToString());
                ...
                {
                    string contactus = "<a href=\"" + RootPath + "/ContactUser.aspx?ID=" + EncryptDecrypt.DESEncrypt(UserID.ToString()) + "&SID=" + EncryptDecrypt.DESEncrypt(CustomID.ToString()) + "&ET=CM\" target=\"_blank\"><img src='" + RootPath + "/images/Dashboard/contactus.gif' alt='Contact Us' border='0'/></a>";
            ...
            catch (Exception ex)
            {
                InBuiltDataBLL objInBuiltData = new InBuiltDataBLL();
                /*** Error Log ***/
                objInBuiltData.ErrorHandling("ERROR", "ViewItem.aspx.cs", "GetBulletinDetails", ex.Message, Convert.ToString(ex.StackTrace),
                    Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
            }
        }
        private void StoreLinks()
        {
            ...
            catch (Exception ex)
            {
                InBuiltDataBLL objInBuiltData = new InBuiltDataBLL();
                /*** Error Log ***/
                objInBuiltData.ErrorHandling("ERROR", "ViewItem.aspx.cs", "StoreLinks", ex.Message, Convert.ToString(ex.StackTrace),
                    Convert.ToString(ex.InnerException), Convert.ToString(ex.Data));
            }
        }
