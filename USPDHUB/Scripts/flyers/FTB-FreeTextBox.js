...
function FTB_FreeTextBox(id, enableToolbars, readOnly, buttons, dropdownlists, breakMode, pasteMode, tabMode, startMode, clientSideTextChanged, designModeCss, designModeBodyTagCssClass, baseUrl, textDirection, buttonImageFormat, imageGalleryUrl, imageGalleryPath, receiveFocus, buttonWidth, buttonHeight) {
    ...
    this.designEditor.document.write("<html" + ((this.textDirection == "rtl") ? " dir='rtl'" : "") + ">" +
			"<head>" +
			((this.designModeCss != '') ? "<link rel='stylesheet' href='../../css/FreeTextBoxStyles.css' type='text/css' />" : "") +
    ...
    ///this.design mode
    /// <reference path="../../css/FreeTextBoxStyles.css" />
...
FTB_FreeTextBox.prototype.CreateLink = function () {
    ...
    if (FTB_LinkPopUpHtml != '') {
        var linkWin = window.open("", "linkWin", "width=350,height=155,left=500");
        if (linkWin) {
            linkWin.focus();
...
FTB_FreeTextBox.prototype.InsertImageFromGallery = function () {
    ...
    var IDID1 = document.getElementById('editDivCheck').value;

    var id = IDID1.replace("edit", "");
    document.getElementById('editDivCheck').value = id;
    var gallery = window.open("FTB_ImageGallery.aspx?fitblockwidth=" + (document.getElementById(IDID1).parentElement.offsetWidth - 22), 'gallery', 'width=800,height=730,toolbars=0,resizable=0');
    gallery.focus();
