...
namespace USPDHUBBLL
{
    public class BaseWeb : System.Web.UI.Page
    {
        ...
        public static string GetHelpcontentbyHelpID(string helpID)
        {
            ...
            USPDHUBBLL.ezSmartSiteWizard ezSmartsiteObj = new USPDHUBBLL.ezSmartSiteWizard();
            DataTable dthelpdetails = ezSmartsiteObj.GetHelpmenuDetailsbyHelpID(int.Parse(helpID));
        ...
        [WebMethod]
        public static List<string> GetHelpItems(string prefixText, bool isLiteVersion)
        {
            CommonBLL objCommon = new CommonBLL();
            List<string> HelpResult = new List<string>();
            try
            {
                HelpResult = objCommon.GetHelpSearchDetails(prefixText, isLiteVersion);                
            }
        ...
        [WebMethod]
        public static string ServerSidefill(string userid)
        {
            ...
            if (userid.Length > 0)
            {
                try
                {
                    // *** Counting Mobile Newsletter alerts ***
                    DataTable dtmobilenewslettercontacts = new DataTable();
                    dtmobilenewslettercontacts = busobj.GetMobileAppAlerts(false, Convert.ToInt32(userid));
                    int messages = dtmobilenewslettercontacts.Rows.Count;

                    // *** Counting Mobile Newsletter alerts ***
                    DataTable dtmobiletips = new DataTable();
                    dtmobiletips = busobj.GetMobileTips(false, Convert.ToInt32(userid));
                    int tips = dtmobiletips.Rows.Count;

                    if (messages > 0 && tips == 0)
                        typevalue = "M." + Convert.ToString(messages);
                    else if (messages == 0 && tips > 0)
                        typevalue = "T." + Convert.ToString(tips);
                    else
                    {
                        if (messages > 0 && tips > 0)
                            typevalue = "MT." + Convert.ToString(messages) + "." + Convert.ToString(tips);
                    }
                }
                catch
                {
        ...
        [WebMethod]
        public static string UpdateCallItemsOrder(string itemOrder)
        {
            ...
            try
            {
                ...
                if (itemOrder.Length > 0)
                {
                    if (HttpContext.Current.Session["UserID"] != null)
                    {
                        ...
                        string[] strOrderIds = itemOrder.Split(',');
                        for (int i = 0; i < strOrderIds.Length; i++)
                        {
                            objAddOn.UpdateCallAddonOrder(Convert.ToInt32(strOrderIds[i]), i + 1, CUserID);
                        }
                        objBus.Insert_SilentPushMessages(Convert.ToInt32(HttpContext.Current.Session["ProfileID"].ToString()), DateTime.Now, false, 0, "PrivateCallAddOns", "OrderNumberUpdate");
                    }
                }
                _result = "success";
            }
        ...
        [WebMethod]
        public static string UpdatePublicCallItemsOrder(string itemOrder)
        {
            string _result = "failed";
            try
            {
                ...
                if (itemOrder.Length > 0)
                {
                    if (HttpContext.Current.Session["UserID"] != null)
                    {
                        ...
                        for (int i = 0; i < strOrderIds.Length; i++)
                        {
                            objAddOn.UpdatePublicCallAddonOrder(Convert.ToInt32(strOrderIds[i]), i + 1, CUserID);
                        }
                        objBus.Insert_SilentPushMessages(Convert.ToInt32(HttpContext.Current.Session["ProfileID"].ToString()), DateTime.Now, false, 0, "PublicCallAddOns", "OrderNumberUpdate");
                    }
                }
                _result = "success";
            }
        ...
        [WebMethod]
        public static string UpdatePSCItemsOrder(string itemOrder)
        {
            string _result = "failed";
            try
            {
                ...
                if (itemOrder.Length > 0)
                {
                    if (HttpContext.Current.Session["UserID"] != null)
                    {
                        ...
                        for (int i = 0; i < strOrderIds.Length; i++)
                        {
                            objPSC.UpdatePSCAddonOrder(Convert.ToInt32(strOrderIds[i]), i + 1, CUserID);
                        }
                        objBus.Insert_SilentPushMessages(Convert.ToInt32(HttpContext.Current.Session["ProfileID"].ToString()), DateTime.Now, false, 0, "PrivateSmartConnectCallAddOns", "OrderNumberUpdate");
                    }
                }
                _result = "success";
            }
