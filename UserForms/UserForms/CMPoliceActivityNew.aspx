<%@ Page Title="" Language="C#" MasterPageFile="~/Admin.Master" AutoEventWireup="true"
    CodeBehind="CMPoliceActivityNew.aspx.cs" Inherits="UserForms.CMPoliceActivityNew"
...
<asp:Content ID="Content1" ContentPlaceHolderID="cphUser" runat="server">
    ...
    <script type="text/javascript">
                                    ...
                                    "<div  id=\"edit" + CID + "\"> " +
                                        "<table style='border-bottom:2px solid gray;' cellpadding=\"2\" cellspacing=\"0\" width='100%'> " +
                                            ...
                                            "<tr>" +
                                                ...
                                                "<td><textarea style='width: 300px; font-weight:bold;' id='txtDescription" + CID + "'></textarea></td>" +
                                            "</tr>" +
                                            "<tr>" +
                                                "<td><span style='font-weight:bold;'>Additional Details</span></td>" +
                                                "<td><textarea style='width: 300px;' id='txtAddDetails" + CID + "'></textarea></td>" +
                                            "</tr>" +
                                            "<tr>" +
        ...
        function DeleteImage(imgID) {
            ...
            if (confirm("Are you sure you want to delete this image?")) {
                ...
                imagesectionID = imagesectionID.replace("_1", "");
                console.log(imagesectionID);
    ...
    <script type="text/javascript">
        ...
        function PreviewHTML(type) {
            ...
            // bulletin & login name
            var fromDate = document.getElementById("<%=txtFromDate.ClientID %>").value.trim().replace("MM/DD/YYYY", "");
            var fromhrs = document.getElementById("<%=txtFromHours.ClientID %>").value.replace("Hour", "12").trim();
            var frommns = document.getElementById("<%=txtFromMinutes.ClientID %>").value.replace("Minutes", "00").trim();
            var fromSS = document.getElementById("<%=ddlFromSS.ClientID %>").value.replace("", "").trim();

            var toDate = document.getElementById("<%=txtToDate.ClientID %>").value.replace("MM/DD/YYYY", "");
            var tohrs = document.getElementById("<%=txtToHours.ClientID %>").value.replace("Hour", "12");
            var toMin = document.getElementById("<%=txtToMinutes.ClientID %>").value.replace("Minutes", "00");
            var toSS = document.getElementById("<%=ddlToSS.ClientID %>").value;

            if (fromDate != "") {
                //HTML
                ROWS = "<tr><td  style='padding-top:5px; padding-left:5px;'>From Date:</td><td   style='padding-left:5px;'>" + fromDate + "&nbsp; " + fromhrs + ":" + frommns + " " + fromSS + "</td></tr>";
                //XML
                XMLString = " FromDate= '" + fromDate + "'  ";
            }
            if (toDate != "") {
                //HTML
                ROWS = ROWS + "<tr><td  style='padding-top:5px; padding-left:5px;'>To Date:</td><td   style='padding-left:5px;'>" + toDate + "&nbsp; " + tohrs + ":" + toMin + " " + toSS + " </td></tr>";
                //XML
            ...
            var firstBorder = "";
            if (ROWS != "")
                firstBorder = " <tr><td colspan='2'><table cellpadding='0' cellspacing='0' style='border: 1px solid black; width: 300px;'>" + ROWS + "</table></td></tr>";
            ...
            previewHTMLStr = "<table cellpadding='0' cellspacing='0' style='padding-left:0px; padding-top:10px;padding-right:5px; text-align:left;'>" + firstBorder + thirdBorder + "</table>";
        ...
        function ValidateCallDetails() {
            ...
            if (Page_ClientValidate('SV') && Page_IsValid) {
                ...
                if (document.getElementById("<%=txtFromDate.ClientID %>").value != "" && document.getElementById("<%=txtToDate.ClientID %>").value != "") {
                    ...
                    var selHours = document.getElementById("<%=txtFromHours.ClientID %>").value;
                    var selmins = document.getElementById("<%=txtFromMinutes.ClientID %>").value;

                    if (document.getElementById("<%=ddlFromSS.ClientID %>").value == 'AM' && selHours == 12)
                        selHours = 0;
                    if (document.getElementById("<%=ddlFromSS.ClientID %>").value == 'PM' && selHours < 12)
                        selHours = 12;

                    fromDate.setHours(selHours, selmins, 0);


                    //To Date Format
                    var todate = new Date(document.getElementById("<%=txtToDate.ClientID %>").value);
                    var toHours = document.getElementById("<%=txtToHours.ClientID %>").value;
                    var tomins = document.getElementById("<%=txtToMinutes.ClientID %>").value;

                    if (document.getElementById("<%=ddlToSS.ClientID %>").value == 'AM' && toHours == 12)
                        toHours = 0;
                    if (document.getElementById("<%=ddlToSS.ClientID %>").value == 'PM' && toHours < 12)
                        toHours = 12;

                    todate.setHours(toHours, tomins, 0);
                    if (selHours != "" && selmins != "" && toHours != "" && tomins != "") {
                        if (parseInt(selHours) > 12 && parseInt(selmins) > 60 && parseInt(toHours) > 12 && parseInt(tomins) > 60) {
                            alert('Invalid time format.');
                            document.getElementById("<%=txtFromHours.ClientID %>").focus();
                            reurnValue = false;
                        }
                        else if (parseInt(selHours) > 12 && parseInt(selmins) > 60) {
                            alert('Invalid time format.');
                            document.getElementById("<%=txtFromHours.ClientID %>").focus();
                            reurnValue = false;
                        }
                        else if (parseInt(toHours) > 12 && parseInt(tomins) > 60) {
                            alert('Invalid time format.');
                            document.getElementById("<%=txtToHours.ClientID %>").focus();
                            reurnValue = false;
                        }
                        else if (parseInt(selHours) > 12) {
                            alert('Invalid time format.');
                            document.getElementById("<%=txtFromHours.ClientID %>").focus();
                            reurnValue = false;
                        }
                        else if (parseInt(selmins) > 60) {
                            alert('Invalid time format.');
                            document.getElementById("<%=txtFromMinutes.ClientID %>").focus();
                            reurnValue = false;
                        }
                        else if (parseInt(toHours) > 12) {
                            alert('Invalid time format.');
                            document.getElementById("<%=txtToHours.ClientID %>").focus();
                            reurnValue = false;
                        }
                        else {
                            if (parseInt(tomins) > 60) {
                                alert('Invalid  time format.');
                                document.getElementById("<%=txtToMinutes.ClientID %>").focus();
                                reurnValue = false;
                            }
                        }
                    ...
                    if (todate < fromDate) {
                        alert('From date should be later than current date.');
                        return false
                    }
    ...
    <script type="text/javascript">
        ...
        function EditImage(imgdivID, parentID) {
            ...
            ifrm.setAttribute("src", "BulletinsForm_ImageGallery.aspx?fitblockwidth=" + (document.getElementById(imgdivID).offsetWidth - 32) + "&imgSrc=" + imgSrc);
                                        ...
                                        <tr>
                                            ...
                                            <td style="height: 30px;">
                                                ...
                                                <table>
                                                    <tr>
                                                        <td>
                                                            <asp:TextBox runat="server" ID="txtFromHours" Enabled="False" Width="30px" MaxLength="2"></asp:TextBox>
                                                            <span style="font-weight: bold;">Hour</span>
                                                            <cc1:TextBoxWatermarkExtender ID="TextBoxWatermarkExtender3" TargetControlID="txtFromHours"
                                                            ...
                                                            <asp:RegularExpressionValidator ID="RegularExpressionValidator7" runat="server" Display="Dynamic"
                                                                ControlToValidate="txtFromHours" ValidationExpression="^(1[0-2]|0?[1-9])" ValidationGroup="SV"
                                                                ErrorMessage="Invalid Time Format">*</asp:RegularExpressionValidator>
                                                        </td>
                                                        <td>
                                                            <asp:TextBox runat="server" ID="txtFromMinutes" Enabled="False" Width="47px" MaxLength="2"></asp:TextBox>
                                                            <span style="font-weight: bold;">Minutes</span>
                                                            <cc1:TextBoxWatermarkExtender ID="TextBoxWatermarkExtender7" TargetControlID="txtFromMinutes"
                                                            ...
                                                            <asp:RegularExpressionValidator ID="RegularExpressionValidator8" runat="server" Display="Dynamic"
                                                                ControlToValidate="txtFromMinutes" ValidationExpression="^[0-5]\d" ValidationGroup="SV"
                                                                ErrorMessage="Invalid Time Format">*</asp:RegularExpressionValidator>
                                                        </td>
                                                        <td>
                                        ...
                                        <tr>
                                            ...
                                            <td style="height: 30px;">
                                                ...
                                                <table>
                                                    <tr>
                                                        <td>
                                                            <asp:TextBox runat="server" ID="txtToHours" Enabled="False" Width="30px" MaxLength="2"></asp:TextBox>
                                                            <span style="font-weight: bold;">Hour</span>
                                                            <cc1:TextBoxWatermarkExtender ID="TextBoxWatermarkExtender1" TargetControlID="txtToHours"
                                                                WatermarkText="Hour" runat="server" WatermarkCssClass="watermarkbulletindate">
                                                            </cc1:TextBoxWatermarkExtender>
                                                            <asp:RegularExpressionValidator ID="RegularExpressionValidator1" runat="server" Display="Dynamic"
                                                                ControlToValidate="txtToHours" ValidationExpression="^(1[0-2]|0?[1-9])" ValidationGroup="SV"
                                                                ErrorMessage="Invalid Time Format">*</asp:RegularExpressionValidator>
                                                        </td>
                                                        <td>
                                                            <asp:TextBox runat="server" ID="txtToMinutes" Enabled="False" Width="47px" MaxLength="2"></asp:TextBox>
                                                            <span style="font-weight: bold;">Minutes</span>
                                                            <cc1:TextBoxWatermarkExtender ID="TextBoxWatermarkExtender8" TargetControlID="txtToMinutes"
                                                                WatermarkText="Minutes" runat="server" WatermarkCssClass="watermarkbulletindate">
                                                            </cc1:TextBoxWatermarkExtender>
                                                            <asp:RegularExpressionValidator ID="RegularExpressionValidator9" runat="server" Display="Dynamic"
                                                                ControlToValidate="txtToMinutes" ValidationExpression="^[0-5]\d" ValidationGroup="SV"
                                                                ErrorMessage="Invalid Time Format">*</asp:RegularExpressionValidator>
                                                        </td>
                                                        <td>
                            ...
                            <div class="fields_wrap ">
                                ...
                                <div class="right_fields" style="width: 470px;">
                                    <table cellpadding="0" cellspacing="0" id='tblExTime'>
                                        ...
                                        <tr>
                                            <td>
                                                <asp:TextBox ID="txtExpires" runat="server" CssClass="txtfild1" onChange="ShowExTimeDiv();"
